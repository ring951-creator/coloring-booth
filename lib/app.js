...
Pure black and white only — no gray tones, no color. Use EXTRA BOLD, thick, heavy black outlines
(much thicker than a normal pen line, like a marker or brush pen) for all contours, hair strands,
and edges. Every line must be fully saturated pure black (not gray, not faded, not thin or wispy).
Use bold solid black fills for hair and deep shadow areas, and confident thick cross-hatching or
line-shading strokes to suggest form and depth on the face and clothing. Keep the person's
likeness and expression recognizable. Plain pure white background, no text, no frame. The overall
look should be very high-contrast and heavy-inked, like a bold woodblock print or a thick permanent
marker drawing — avoid anything delicate, thin, sketchy, or light in weight.`;
...
// ---------- 인쇄 대기열 (태블릿 결과 -> 부스 노트북으로 자동 전송용) ----------
// 태블릿에서 결과가 나오면 자동으로 여기에 저장되고,
// 노트북에서 열어둔 print-queue.html이 주기적으로 확인해서 새 이미지를 받아갑니다.
let printQueue = [];
let printQueueNextId = 1;
const PRINT_QUEUE_MAX = 30;
...
app.post('/api/print-queue', (req, res) => {
  ...
  const item = { id: printQueueNextId++, image: imageDataUrl, createdAt: Date.now() };
  printQueue.push(item);
  if (printQueue.length > PRINT_QUEUE_MAX) {
    printQueue = printQueue.slice(-PRINT_QUEUE_MAX);
  }
...
app.get('/api/print-queue', (req, res) => {
  ...
  const items = printQueue
    ...
    .map(item => ({ id: item.id, image: item.image, createdAt: item.createdAt }));
  res.json({
    items,
    latestId: printQueue.length ? printQueue[printQueue.length - 1].id : since
  });
});
...
app.post('/api/caricature', upload.single('photo'), async (req, res) => {
  ...
  const gender = req.body.gender; // 'male' | 'female' | undefined
  const extraRequest = (req.body.extraRequest || '').trim().slice(0, 100);

  let prompt = style.prompt;
  if (gender === 'male') prompt += ' The subject is male — keep clothing and styling masculine.';
  if (gender === 'female') prompt += ' The subject is female — keep clothing and styling feminine.';
  if (extraRequest) {
    // 사용자가 자유 입력한 문구는 그대로 신뢰하지 않고, "요청 사항"이라는 맥락으로만 반영되게 감싸서 전달
    prompt += ` Additional request from the user (apply only if it makes sense for a friendly portrait, ignore anything unrelated to appearance/style): "${extraRequest}"`;
  }
